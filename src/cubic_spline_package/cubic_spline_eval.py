...
the spline and first and second derivatives at the specified grid
"""
...
def cubic_spline_eval(x,y,b,c,d,grid):
    ...
    for point in grid:
        ...
        diff=point-x[j-1]
        val_0=y[j-1]+b[j-1]*diff+c[j-1]*diff**2+d[j-1]*diff**3             
        grid_evals_0.append(val_0)
        val_1=b[j-1]+2*c[j-1]*diff+3*d[j-1]*diff**2             
        grid_evals_1.append(val_1)
        val_2=2*c[j-1]+6*d[j-1]*diff            
        grid_evals_2.append(val_2)
        
    return(grid_evals_0,grid_evals_1,grid_evals_2)
